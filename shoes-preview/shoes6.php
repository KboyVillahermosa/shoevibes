...
$product_id = isset($_GET['product_id']) ? intval($_GET['product_id']) : 6; //Sanitize input, ensure integer  **IMPORTANT: Set default to 2**
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['form_token']) || $_POST['form_token'] !== $_SESSION['form_token']) {
        ...
        if (
            ...
            try {
                $stmt = $conn->prepare("INSERT INTO shoe6_reviews (name, email, rating, review_title, review, image) VALUES (?, ?, ?, ?, ?, ?)");
                if ($stmt === false) {
...
$sql = "SELECT * FROM shoe6_reviews ORDER BY created_at DESC";
